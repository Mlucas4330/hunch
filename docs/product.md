...
packs on the home page, R$47 for one analysis, R$147 for three, and a webhook grants once the
...
- **Credits are not a subscription.** A page owner audits twice a year, and a monthly plan *on that
  ...
  **A monitoring subscription is the shape to avoid here.** Billing monthly to re-measure the page
  ...
  Paid acquisition arithmetic is answered by the price of the analysis instead: one purchase has to
  repay one click on its own. See [ads.md](ads.md).
...
- **The price is set against what the analysis is worth, not against the token bill.** An analysis costs a couple of
  reais to produce and sells for R$47. The margin is not the point; the point is that the price is
  set against what the analysis is worth to the buyer, and R$19 was too little to read as one.

  It was set against paid acquisition once, at R$147, on the arithmetic in [ads.md](ads.md). No
  campaign is running now, so that constraint is not the one deciding the number.
