...
const marketRules = (market: string) => `- This product sells in ${market}. Every recommendation must
  be something the owner can actually implement there: never suggest a payment method, an
...
- NEVER invent how this product is sold. Subscriptions, free trials, refunds, cancellation terms and
  ...
  owner's own details state them. A guarantee recommended for a product that has no subscription is
 ...
 */
/**
 * What may be said about the marks a page shares with pages built out of the same defaults.
 *
 * **Shared for the reason `marketRules` and `competitorRules` are**: the risk is identical wherever
 * these counts reach a model, and three wordings of one rule drift the first time one is edited.
 *
 * Two prohibitions, and they fail differently:
 *
 * - **Origin.** "Your page was generated by AI" is a claim about how the page was made, and nobody
 *   measured that. A hand-written page has gradients and stock icons too. The counts are evidence of
 *   shared defaults and never of authorship, and the product's own landing page makes the same
 *   distinction -- naming a builder is context, asserting things about its output is not.
 * - **Outcome.** "Replacing the gradient will build trust" is a result nobody controlled for, and it
 *   is the delta rule reaching the surface where it is most tempting, because the advice sounds
 *   obviously right. See docs/invariants.md.
 *
 * The second is why this exists at all rather than being left to `evidenceRules`: that rule bans
 * invented *numbers*, and the sentence at risk here carries no number.
 */
...
- **These are choices, not defects, and the reader was shown them without a grade.** Nothing you
  write may call them problems, errors, or mistakes. A page is allowed to have a gradient.
- **NEVER say or imply the page was generated, built by AI, or made with any particular tool**, and
  never guess at one. You do not know how this page was made and neither does anything that counted
  these. The single exception is that the page itself declared a builder, and even then the fact is
  "the page says so", never "we can tell".
- **NEVER say what changing one of these will produce.** No lift, no trust gained, no conversion, no
  "visitors will take it more seriously". Nobody measured any of that here.
- What you MAY do is argue the mechanism, in one specific sentence about THIS page: three cards that
  each say a generic noun tell a visitor nothing they could not have guessed, and a button reading
  "Get started" does not say what happens next. Then write the specific replacement.
- Use the category "distinctiveness" for a fix that answers one of these, and set finding to null:
  none of these marks is a finding you may attach to.`
...
- Stop at the last concrete thing you have to say. Never close a field with encouragement, with a
  summary of what you just wrote, or with what the owner stands to gain.`
...
the exact words a visitor would read on the page. It is never an instruction to the owner. Do not
...
You may also receive a "Business details" brief written by the owner. When present, treat every
...
- Rewrite using the page's OWN real claims plus any Business details provided. NEVER invent
  ...
  the brief.
- When a variant needs a specific the owner has NOT supplied (a metric, a customer quote, a logo,
...
return is a line you are telling the owner to change.
...
policy, a way to reach the company). You may also be given business details written by the owner.
...
- problem is ONE sentence (about 20 words or fewer) naming what the current flow costs the visitor.
  No fixes inside it.
- steps is 2 to ${PLAYBOOK_STEPS_MAX} entries. Each is ONE concrete action the owner performs on
...
- category is the conversion blocker the fix removes. Use signup_friction for auth and form cost,
  ...
  for what the page does wrong in a phone viewport, performance for what the page costs to load, and
  distinctiveness for a page that says nothing a visitor could not have guessed from any other page
  in its category.
...
- mobile and performance are measured, so a fix in either MUST name the finding it answers. They are
  ...
  inside the owner's own control: a step is "serve the hero image at the size it renders" or "set a
...
- evidence is ONE sentence explaining the CRO reasoning behind the fix, grounded in what the
  ...
  the fix removes.
- Treat any business details from the owner as ground truth and make the steps fit their real
...
crawlers that feed AI answers. You may also be given business details written by the owner.
...
- Treat any business details from the owner as ground truth. Never invent facts about the product.
${impactScoreRules()}
${readoutRules()}
${evidenceRules()}
${marketRules(market)}
...
- **The replacement asserts something no page here says.** A number, a guarantee, a claim about the
  ...
  to get wrong, because the owner pastes it onto a live page.
...
2 more alternates for the SAME section, so the owner can swap the recommendation for a different
