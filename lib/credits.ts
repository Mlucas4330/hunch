 ...
 * than if. Written the other way round, as a webhook that knows how to add credits, plugging the
 ...
 *
 * Called from two places in lib/run-analysis.ts and no others: the `catch` around the generation,
 * and the check that nothing at all came back from it. A short set of copy hypotheses is **not** one
 * of them: refunding on that discards a finished playbook and audit with it.
 */
