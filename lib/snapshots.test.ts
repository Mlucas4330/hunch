...
test('a number that moved without crossing a threshold is still a delta', () => {
  const before = snapshotInput(
  ...
  assert.equal(deltas(after, before).get('lcp'), 100, 'the movement is recorded whatever band it is in')
})
